...
export async function cacheMovie(source, sourceId) {
  ...
  if (isStale) {
    ...
    const upsert = db.prepare(`
      INSERT INTO movies (source, source_id, title, poster, backdrop, synopsis, duration, note, genres, release_date, updated_at)
      VALUES (@source, @source_id, @title, @poster, @backdrop, @synopsis, @duration, @note, @genres, @release_date, datetime('now'))
      ON CONFLICT(source, source_id) DO UPDATE SET
